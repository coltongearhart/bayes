...
*Said another way, we can use a sample from $P(\theta \mid \mathbf{y})$ to approximate the actual posterior distribution. This empirical density can be used to estimate any quantity of interest, e.g. $P(\theta < \theta_0) = \text{??} \rightarrow $ just look at percent of sample that meets this condition.*
...
# -> emp.hpd() uses posterior sample to estimate bounds rather than using a density function to find the exact bounds like hpd() does
